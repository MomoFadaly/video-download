...
Use when downloading a video from YouTube or other sites, extracting audio, downloading subtitles, or getting video metadata without downloading.

## Install

```bash
# Claude Code plugin install (one-line)
claude plugin install video-download --from https://fadaly.net/downloads/skills/video-download.zip
```

Or clone this repo into your Claude skills directory:

```bash
git clone https://github.com/MomoFadaly/video-download.git ~/.claude/skills/video-download
```

Or download the zip from [fadaly.net/skills/video-download](https://fadaly.net/skills/video-download) (the per-skill landing page on fadaly.net) and extract into `~/.claude/skills/`.
...
- yt-dlp (github.com/yt-dlp/yt-dlp) — primary
- ffmpeg — required for merging streams
